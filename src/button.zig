...
pub fn create(hInstance: w.HINSTANCE, parent: Window, allocator: *std.mem.Allocator) !Self {
    const windowConfig = Window.WindowParameters{ .title = toUtf16const("Click me!"), .className = toUtf16const("BUTTON"), .width = 100, .height = 25, .style = w.WS_TABSTOP | w.WS_VISIBLE | w.WS_CHILD | w.BS_DEFPUSHBUTTON, .parent = parent, .register_class = false };
